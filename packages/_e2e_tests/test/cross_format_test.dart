...
String _plain(Delta d) =>
    d.operations.where((op) => op.data is String).map((op) => op.data as String).join();
...
void main() {
  ...
  group('ConverterRegistry auto-dispatch', () {
    ...
    test('typed lookup returns correct exporter', () async {
      final exp =
          registry.exporter<String, HtmlOptions>('html');
  ...
  group('format chains', () {
    ...
    test('docx -> pdf via Delta intermediate', () async {
      ...
      expect(latin1.decode(pdfBytes, allowInvalid: true),
          contains('Pipeline'));
    });
